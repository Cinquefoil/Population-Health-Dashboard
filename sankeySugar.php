...
UNION
SELECT DISTINCT(`SugarHigh`) AS name FROM `ktphalldata`
UNION
SELECT DISTINCT(CONCAT(SugarHigh,'-F')) AS name FROM `ktphalldata`
UNION
...
";
    ...
    $myquery = "
SELECT `scnZone` AS source, `SugarHigh` AS target,
COUNT(*) as value
FROM `ktphalldata`
WHERE `revisit` = 'TRUE' AND `visitFY` = 'FY2013'
GROUP BY source, target
UNION
SELECT `SugarHigh` AS source, `action` AS target,
COUNT(*) as value
FROM `ktphalldata`
WHERE `revisit` = 'TRUE' AND `visitFY` = 'FY2013'
GROUP BY source, target
UNION
SELECT `action` AS source, CONCAT(SugarHigh,'-F') AS target,
COUNT(*) as value
FROM `ktphalldata`
