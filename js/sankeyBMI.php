...
UNION
SELECT DISTINCT(`BMIGrp`) AS name FROM `ktphalldata`
UNION
SELECT DISTINCT(CONCAT(BMIGrp,'-F')) AS name FROM `ktphalldata`
UNION
...
";
    ...
    $myquery = "
SELECT `scnZone` AS source, `BMIGrp` AS target,
COUNT(*) as value
FROM `ktphalldata`
WHERE `revisit` = 'TRUE' AND `visitFY` = 'FY2013'
GROUP BY source, target
UNION
SELECT `BMIGrp` AS source, `action` AS target,
COUNT(*) as value
FROM `ktphalldata`
WHERE `revisit` = 'TRUE' AND `visitFY` = 'FY2013'
GROUP BY source, target
UNION
SELECT `action` AS source, CONCAT(BMIGrp,'-F') AS target,
COUNT(*) as value
FROM `ktphalldata`
